  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.scroll}>
      ...
      <Text style={styles.caption}>
        Clear HLS streams over the network. DRM demo uses Widevine (Android) or FairPlay (iOS). For
        offline progressive MP4s, use the Downloads tab: queue downloads (one at a time), then open
        a finished file here under Offline file. Picture-in-picture: use the button below or leave
        the app while playing. After adding the module, run Expo prebuild (or a dev client build) so
        native PiP changes apply.
      </Text>

      <View style={styles.modeRow}>
        <Pressable
          style={[styles.modeBtn, playbackMode === "clear-hls" && styles.modeBtnActive]}
          onPress={() => {
        ...
        <Pressable
          style={[styles.modeBtn, playbackMode === "drm-demo" && styles.modeBtnActive]}
          onPress={() => {
        ...
        >
          <Text
            style={[styles.modeBtnText, playbackMode === "drm-demo" && styles.modeBtnTextActive]}
          >
            DRM demo
          </Text>
        </Pressable>
        <Pressable
          style={[styles.modeBtn, playbackMode === "offline" && styles.modeBtnActive]}
          onPress={() => {
      ...
      {showVideo ? (
        <View style={styles.playerWrap}>
          <MediaPlayer
            ...
            onBuffer={({ isBuffering }: OnBufferData) => onBufferingChange(isBuffering)}
        ...
        <View style={[styles.playerWrap, styles.offlinePlaceholder]}>
          <Text style={styles.offlinePlaceholderText}>
            Go to the Downloads tab, queue at least one video, then tap a finished row to play it
            here — or choose Offline file after a file is on device.
          </Text>
        </View>
      )}
      ...
      {showVideo ? (
        <View style={styles.pipRow}>
          ...
          <Text style={styles.pipHint}>
            Leaving the app while video plays can auto-enter PiP when the OS supports it.
          </Text>
        </View>
      ) : null}
      ...
      {Platform.OS === "ios" && playbackMode === "drm-demo" ? (
        <Text style={styles.hint}>
          ...
          <Text style={styles.mono}>certificateUrl</Text>, <Text style={styles.mono}>licenseServer</Text>
          , optional <Text style={styles.mono}>contentId</Text>.
        </Text>
      ) : null}
    </ScrollView>
  );
